...
class AuthManager {
  ...
  async syncUserData() {
    ...
    try {
      ...
      const localWishlist = JSON.parse(
        localStorage.getItem('beautyFindWishlist') || '[]',
      );
      const localOwned = JSON.parse(
        localStorage.getItem('beautyFindOwned') || '[]',
      );
  ...
  promptGuestToSignUp() {
    ...
    notification.className =
      'fixed bottom-4 right-4 bg-accent text-primary p-4 rounded-lg shadow-lg max-w-sm z-50';
  ...
  onGuestAction(action) {
    ...
    if (actionsToPrompt.includes(action)) {
      ...
      const interactionCount =
        parseInt(localStorage.getItem('guestInteractions') || '0') + 1;
...
class AuthModalManager {
  ...
  setupEventListeners() {
    ...
    document
      ...
      .addEventListener('click', () => this.handleSocialSignIn('facebook'));
  ...
  async handleSocialSignIn(provider) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (window.app) {
          window.app.showNotification(
            `Welcome! Signed in with ${provider}`,
            'success',
          );
          window.app.updateWishlistCounter();
        }
